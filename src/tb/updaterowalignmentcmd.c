...
UpdateRowAlignmentCmd::UpdateRowAlignmentCmd(Table *t, TableViewer *v, 
		TextAlign::Type a): UpdateVectorAlignmentCmd(t, v, a) {
	List<CellRow *> selection;
	v->GetSelectedRows(&selection);
	for (selection.first(); !selection.done(); selection.next()) {
		CellRow *row = selection.cur();
		TextAlign::Type sal = row->GetAlignment();
		if (sal != a) {
			GetVectors()->add(row);
