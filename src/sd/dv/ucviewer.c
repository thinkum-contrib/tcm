...
#include "ucupdateactortypecmd.h"
#include "ucviewer.h"
 
UCViewer::UCViewer(Config *c, UCWindow *d): ERViewer(c, d) { }

void UCViewer::UpdateNodeShapeType(int actorType) {
	...
	if (GetCurView()->NrSelected() > 0) {
		NewCommand(new UCUpdateActorTypeCmd(
